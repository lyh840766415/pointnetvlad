...
BASE_PATH=os.path.join(BASE_DIR, '../pointNetVlad_backup/benchmark_datasets/')
print(BASE_PATH)
...
def load_pc_file(filename):
	...
	pc = np.loadtxt(os.path.join(BASE_PATH,filename),delimiter=" ",dtype=np.float64)
	'''
	print(pc)
	print(pc.shape)
	'''
	...
	if(pc.shape[0]!= 4096 or pc.shape[1]!= 3):
		...
		return np.array([])
	'''
	print("pointcloud shape correct")
	exit()
